...
namespace EFProviderWrapperToolkit
{
    ...
    {
        private static ConcurrentDictionary<string, MetadataWorkspace> metadataWorkspaceMemorizer = new ConcurrentDictionary<string, MetadataWorkspace>();
        ...
        public static EntityConnection CreateEntityConnectionWithWrappers(string entityConnectionString, params string[] wrapperProviders)
        {
            ...
            MetadataWorkspace workspace =
                metadataWorkspaceMemorizer.GetOrAdd(
                    ecsb.ConnectionString,
                    key => CreateWrappedMetadataWorkspace(ecsb.Metadata, wrapperProviders));
            ...
            for (int i = 1; i < wrapperProviders.Length; i++)
            {
                connectionString += DbConnectionWrapper.WrappedProviderKey + wrapperProviders[i] + ";";
            }

            connectionString += DbConnectionWrapper.WrappedProviderKey + ecsb.Provider + ";";
            connectionString += ecsb.ProviderConnectionString;

            var topConnection = DbProviderFactories.GetFactory(wrapperProviders[0]).CreateConnection();
            topConnection.ConnectionString = connectionString;

            var newEntityConnection = new EntityConnection(workspace, topConnection);
